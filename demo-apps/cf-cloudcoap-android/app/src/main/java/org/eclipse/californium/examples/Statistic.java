...
public class Statistic {
    ...
    public Statistic(String values) {
        ...
        if (split.length == 17) {
            ...
            tx = parse(split[6]);
            rtt = parse(split[7]);
            lastSuccess = parse(split[8]);
            lastFailure = parse(split[9]);
            lastConnect = parse(split[10]);
            lastRetries = parse(split[11]);
            lastRestart = parse(split[12]);
            rxLast = parse(split[13]);
            txLast = parse(split[14]);
            rxAll = parse(split[15]);
            txAll = parse(split[16]);
        }
    }
    ...
    public String toString() {
        ...
        builder.append(txAll).append(SEP);
        return builder.toString();
    }
    ...
    public static Statistic updateStatistic(Context context, long startTime, boolean restart, CoapTaskResult result) {
        ...
        } else if (result.error != null && result.error.getMessage().contains("EPERM")) {
